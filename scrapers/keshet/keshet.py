...
2. Per-branch, per-category product catalogue (appId=4, offset pagination):
     GET /v2/retailers/1219/branches/{bid}/categories/{catId}/products
...
- The old /v2/retailers/1219/products global endpoint is capped and misses many
  products (e.g. eggs, fresh chicken). Per-branch/per-category avoids the cap.

Architecture
------------
Call ``scrape()`` to get a ``ScrapeResult`` TypedDict compatible with all other
chain scrapers.  Pass a ``ScrapeFilter`` to restrict by name, category, or barcode.
"""
...
# ---------------------------------------------------------------------------
# Top-level categories for Keshet Teamim (discovered from data.js 2026-03).
# All 36 tree-level categories are included; the scraper probes each one per
# branch and skips categories that return 0 products for that branch.
# Names are derived from branch aisleName where available, else a placeholder.
# ---------------------------------------------------------------------------
...
async def _fetch_branch_products(
    ...
    *,
    ...
    for cat_id, cat_name in MAIN_CATEGORIES:
        ...
        if total == 0:
            logger.debug(
                "keshet: branch=%s category %s (%s) — 0 products",
                branch_id,
                cat_id,
                cat_name,
            )
        ...
        logger.info(
            "keshet: branch=%s category %s (%s) — %d products",
            branch_id,
            cat_id,
            cat_name,
            total,
    ...
    logger.info(
        "keshet: branch=%s (%s) — %d unique products across all categories",
        branch_id,
        branch.get("name", ""),
        len(all_products),
    )
...
async def scrape(
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        async def _scrape_one_branch(branch: Branch) -> tuple[str, List[UnifiedProduct], Optional[str]]:
            ...
            logger.info(
                "keshet: branch=%s (%s) — %d unique active products",
                branch["id"],
                branch.get("name", ""),
                len(products),
            )
