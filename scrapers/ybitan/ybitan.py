...
2. Per-branch, per-category product catalogue (appId=4, offset pagination):
     GET /v2/retailers/1131/branches/{bid}/categories/{catId}/products
...
- The old /v2/retailers/1131/products global endpoint is capped and misses many
...
# ---------------------------------------------------------------------------
# Top-level categories for Yenot Bitan (discovered from data.js 2026-03).
...
async def _fetch_branch_products(
    ...
    *,
    ...
    for cat_id, cat_name in MAIN_CATEGORIES:
        ...
        if total == 0:
            logger.debug(
                "ybitan: branch=%s category %s (%s) — 0 products",
                branch_id,
                cat_id,
                cat_name,
            )
        ...
        logger.info(
            "ybitan: branch=%s category %s (%s) — %d products",
            branch_id,
            cat_id,
            cat_name,
            total,
    ...
    logger.info(
        "ybitan: branch=%s (%s) — %d unique products across all categories",
        branch_id,
        branch.get("name", ""),
        len(all_products),
    )
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        if branches is None:
            ...
            if live_branches:
                ...
                logger.info("ybitan: using %d live branches from API", len(branches))
            else:
                branches = ONLINE_BRANCHES
                logger.warning(
                    "ybitan: live branch discovery failed; falling back to %d static branch(es)",
                    len(branches),
                )
        ...
        async def _scrape_one_branch(branch: Branch) -> tuple[str, List[UnifiedProduct], Optional[str]]:
            ...
            logger.info(
                "ybitan: branch=%s (%s) — %d unique active products",
                branch["id"],
                branch.get("name", ""),
                len(products),
            )
